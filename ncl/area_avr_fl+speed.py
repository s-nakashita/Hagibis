...
for k in range(len(sstlist)):
    ...
    wfile = "wspd_avr_core_"+init+suffixes[sst]+".txt"
    ...
    color=colors[sst], edgecolor='white', label=sst, zorder=1)
    ndate = wdata.size
    #tdelta = timedelta(hours=3)
    wdates = [base + tdelta*i for i in range(ndate)]
    ax[1].plot(wdates, wdata, linewidth=2.0, color=colors[sst])
tdelta = timedelta(hours=6)
...
ax[1].set_ylabel('10 m wind (m/s)')
#ax[0].set_title('average within 200 km radius')
fig.tight_layout()
fig.savefig("fl+wspd_avr_core_"+init+".png")