...
if exp == "clim":
    ...
    exit()
outdir = Path('./')
insfcnc = f'fcst_surf_asia_{init}.nc'
inplnc = f'fcst_p_asia_{init}.nc'
outnc   = f'np_fcst_uv_asia_{init}.nc'
...
with trackf.open() as track:
    for l in track:
        ...
        udata = xr.DataArray(unp.reshape(1,nlat,nlon),\
                    ...
                    ('lat',latin),('lon',lonin)],\
                    attrs=attrs_u,name=var_sfc[0]+"10m")
        vdata = xr.DataArray(vnp.reshape(1,nlat,nlon),\
                    [('time',pd.date_range(date,periods=1)),\
                    ('lat',latin),('lon',lonin)],\
                    attrs=attrs_v,name=var_sfc[1]+"10m")
        logging.debug(udata)
        logging.debug(vdata)
        daout.append(udata)
        daout.append(vdata)
