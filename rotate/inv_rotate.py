...
outnc   = 'inv_' + yyyymmddhh + '_mean.nc'
#innc    = 'anl.nc'
#outnc   = 'np_ve_anl.nc'
...
with trackf.open() as track:
    for l in track:
        ...
        logging.debug(f"output grid size: {nmsk}")
        lontc = np.zeros(nmsk)
        lattc = np.zeros(nmsk)
        k = 0
        for j in range(nlat):
        ...
        #exit()
        dataout = xr.DataArray(val,\
            [('time',pd.date_range(date,periods=1)),\
            ('latitude',latout),('longitude',lonout)],\
        ...
        utc,vtc = librotate.xyzd2uv(xdtc,ydtc,zdtc,lontc,lattc)
        #debug
