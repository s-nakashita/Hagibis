...
function plot_track() {
	...
	if [ -f ${tracktxt} ]; then
	    ...
	    gmt psxy -R -J -O tmp_n.txt -i0,1,2,3 -C$CDIR/track_tigge.cpt -Scc -Wfaint -K >> ${outfile}
	    gmt psxy -R -J -O tmp_p.txt -i0,1,2,3 -C$CDIR/track_tigge.cpt -SA -Wfaint -K >> ${outfile} 
	fi
#  rm -f tmp.txt
    done
}
