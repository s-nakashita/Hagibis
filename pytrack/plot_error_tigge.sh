#!/bin/sh
outfile=error-slp-tigge.ps

gmt psbasemap -R2019-10-05T00/2019-10-13T00/0/60 -JX15c/10c -Y8c -Bxa1df6hg1d -Bya20f20g10 -K > ${outfile}
# mean error
#awk '$4 % 6 == 0{print $1 "-" $2 "-" $3 "T" $4, $7*0.01}' ${bstfile} > tmp.txt
ln -fs ./error-slp-mean.txt tmp.txt
...
for orig in ecmf rjtd kwbc egrr; do
  ln -fs ./error-slp-${orig}.txt tmp.txt
...
gmt pslegend -R -J -Dn0.7/0.7+w4c -F -K -O < legend_error_tigge.txt >> ${outfile}
