...
while t0 <= tmax:
    ...
    data = np.loadtxt(f"me_v{init}_vonly_mod.txt")
    stat[0,:,i]  = data[:,0]
    stat[1,:,i] = data[:,1]
    stat[2,:,i]  = data[:,2]
    stat[3,:,i] = data[:,3]
...
for i in range(4):
    ...
    for j in range(len(center)):
        ax.bar(base, stat[i,j], width=width, bottom=0.0, color=colors[j], label=center[j])
...
#for ax in axs[1,:]:
    ...
    fig.savefig(f"{val}_tigge_mod.png")
